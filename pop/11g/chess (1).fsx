    ...
    member this.availableMoves (board : Board) : (Position list * chessPiece list) =
        ...
        if this.nameOfType = "rook" then
            ...
            let rec loop = function
                | (8,_)                               -> ()
                | (x,8)                               -> loop (x+1,0)
                | (x,y) when board.Item (x,y) <> None -> xs <- unSome (board.Item (x,y)) :: xs ; loop  (x,y+1)
                | (x,y)                               -> loop (x,y+1)
        
            loop (0,0)

            let mutable opposite = []
            
            for i=0 to xs.Length-1 do
                if this.color <> xs.[i].color then
                    opposite <- xs.[i] :: opposite
         
            let mutable illegalMoves:Position list = []

            for i=0 to opposite.Length-1 do
                illegalMoves <- fst (board.getVacantNNeighbours (opposite.[i])) @ illegalMoves

            let pMoves = fst (board.getVacantNNeighbours this)
            let mutable moves = []

            for i=0 to pMoves.Length-1 do
                if not (List.contains pMoves.[i] illegalMoves) then
                    moves <- pMoves.[i] :: moves
...
and Board () =
  ...
  /// find the list of all empty squares and list of neighbours
    ...
    member this.returnPieces (col: Color) =
        let unSome k =
            let z = Option.get k
            z
        let mutable myPieces = []
        let mutable xs = []    
        for i=0 to 7 do
            for j=0 to 7 do
                match (this.Item(i,j)) with
                | Some x -> xs <- (unSome(this.Item(i,j)))::xs ; xs 
                | Null -> xs 
                
        for i=0 to xs.Length-1 do
            if col = xs.[i].color then
                myPieces <- xs.[i] :: myPieces 
        myPieces
